  ...
  const handleRegister = () => {
    ...
    switch (activeTab) {
      case 0:
        ...
        // redirect to wa
        window.open(
          `https://wa.me/628882551944?text=${encodeURIComponent(registerForm)}`
        );
        break;
      case 1:
    ...
    <>
      <Head>
        ...
        <meta
          ...
          content={`Bergabunglah dengan kegiatan ${data.activity.name} yang akan diadakan pada tanggal ${data.activity.date} di ${data.activity.location}. Temukan informasi lengkap tentang kegiatan ini di sini.`}
        />
        <meta property="og:image" content={data.activity.image.preview} />
        <meta
          property="og:url"
        ...
        <meta
          ...
          content={`Bergabunglah dengan kegiatan ${data.activity.name} yang akan diadakan pada tanggal ${data.activity.date} di ${data.activity.location}. Temukan informasi lengkap tentang kegiatan ini di sini.`}
        />
        <meta name="twitter:image" content={data.activity.image.preview} />
        <link
          rel="canonical"
      ...
      <div className="container mx-auto py-6 px-4 md:px-20">
        <div className="grid grid-cols-1 md:grid-cols-2 p-4 gap-4 mt-16 lg:mt-8 border-2 rounded-lg">
          <div className="flex flex-col gap-4 w-full order-2 md:order-1">
            ...
            <div className="flex flex-row gap-2">
              <IconCalendar />
              <p className="text-black font-semibold">{data.activity.date}</p>
            </div>
            <div className="flex flex-row gap-2">
              <IconClock />
              <p className="text-black font-semibold">{data.activity.time}</p>
            </div>
          </div>
