...
export default function Index() {
    ...
    const getData = async () => {
        const response = await api.get(
            ActivitiesModel.endpoints.detail + "/" + router.query.id
        );
    ...
    const handleUpdate = async () => {
        const response = await api.patch(
            ActivitiesModel.endpoints.update,
            data
        );
        if (response.status == 200) {
            router.push(ActivitiesModel.url.list);
        }
    };
            ...
            <div className="p-4 sm:ml-64 bg-gray-100 min-h-screen">
                <div className="p-4 flex flex-col gap-4 border-2 bg-white rounded-lg dark:border-gray-700 mt-20">
                    ...
                    <Form
                        ...
                        data={data}
                        postConfig={ActivitiesModel.createConfig}
                    />
                    <div className="mt-4">
                        <button
